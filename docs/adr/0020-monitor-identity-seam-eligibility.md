...
## Amendment — 2026-09-17 (bezel Y joins the identity-keyed memory family)

The bezel tab's parked height (`bezel_y_ratio` 0..1, fraction of the tab's vertical travel) is remembered under the same identity-first pattern as edge/mode/width: `save_bezel_y_ratio` / `load_bezel_y_ratio_identified` in the `db` dock-memory seam, resolved through `resolve_bezel_y_ratio` on every bezel placement (`dock`, edge-switch `reposition`, driver `settle_mode`) and persisted per display on drag release. Broken rows read as absent and the tab centers; a disconnected display centers without persisting, so the tab is never stranded off-screen. Side stays the existing Left/Right edge memory — no new side store.
