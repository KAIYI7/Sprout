...
Bezel placement lives in the same `quick_window`/`appbar` owner but outside the auto-hide motion driver: entering bezel settles once to a zero-width reservation with the collapsed tab placed synchronously, and the driver's per-tick pass returns immediately for bezel — no slide, no reveal gate (open stays click-gated; ticket 217 owns the interaction). The driver-owns-motion boundary is unchanged: bezel adds no second placement writer and no hover trigger.
